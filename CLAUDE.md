...
**Last session date:** 2026-04-16 (session 16)
**Last session summary (session 16):** AI-jobs Tier-1 accuracy — Wave 1 (all 5 items: title patterns, designation↔topic, JD scanner on Tier-1, topic↔anchor, self-rejection prompts). **Item #1:** added ~50 new `_NON_AI_TITLE_PATTERNS` covering the "needs AI knowledge but isn't AI" category — sales engineer / pre-sales / solutions engineer, business development, partnerships, program manager / TPM / chief of staff, product marketing / growth / demand-gen, policy analyst / AI ethicist / governance, IR / RevOps / treasury, community manager / DevRel-non-code, technical writer / UX writer, instructor / curriculum designer, UX designer / UI designer / product designer / UX researcher, AppSec / InfoSec / SOC analyst, IT support / help desk / sysadmin, video producer / creative director, clinical reviewer / medical writer, localization manager, physical security, vendor / sourcing manager, customer solutions architect (sales-side), TAM. **Item #2:** new `_enforce_designation_topic_consistency` — `designation=="Other"` ⇒ `topic=[]`; AI-adjacent designations (AI PM, AI Solutions Architect, AI Developer Advocate, Prompt Engineer) capped at 1 topic. **Item #3:** confirmed `has_non_ai_jd_signals` already runs on Tier-1 sources before tier branch (shipped in RCA-026). **Item #4:** new `_TOPIC_ANCHORS` dict + `_enforce_topic_anchors` — each Gemini-assigned topic must have ≥1 anchor phrase in JD body, else stripped. 15 topics × 5–15 anchors each, all multi-word phrases or unambiguous brands (no 3-letter substrings). **Item #5:** added SELF-REJECTION block to both `jobs_extract_system.txt` and `jobs_extract_lite_system.txt` — explicit list of 21 non-AI role categories that should return `topic:[]` even at AI-first companies; "Experience with ML/AI as a requirement ≠ AI role" guidance; bare Manager/Director/VP titles without AI anchor must self-reject. Motivating example: PhonePe "Manager, Legal" ingested with Topic="Applied ML" because JD said "LLB / LLM from a recognized university" and Gemini conflated the law degree with Large Language Model. **Four-layer fix:** (1) `_NON_AI_TITLE_PATTERNS` in [jobs_ingest.py](backend/app/services/jobs_ingest.py) expanded — `legal manager`, `manager, legal`, `corporate counsel`, `compliance manager`, `contracts manager`, `benefits administration`, `merchant kyc`, `kyc analyst`, + 7 more. (2) New `has_non_ai_jd_signals(jd_html)` helper — two-gate rule: ≥2 legal/HR cluster hits (LLB, PQE, Indian Contract Act, law firm, procurement contracts, MSA, NDA, redlining) AND zero AI signals (pytorch, tensorflow, transformer, MLOps, fine-tuning, RAG, Claude/Gemini/GPT). Wired into `_stage_one` after title filter. (3) Removed `or ["Applied ML"]` fallback in `_validate()` and `enrich_job_lite()` at [jobs_enrich.py:287](backend/app/services/jobs_enrich.py#L287) and [:390](backend/app/services/jobs_enrich.py#L390) — empty topic now propagates as `[]`. (4) Added DISAMBIGUATION block to both `jobs_extract_system.txt` and `jobs_extract_lite_system.txt` telling Gemini that `LLM`-topic = Large Language Model ONLY; LLB/LLM degree/law firm/PQE ⇒ return `"topic": []`, `"designation": "Other"`. Tests: 6 new legal title cases + 5 JD-scanner cases + 1 `_stage_one` integration test in `test_jobs_cost_opt.py`. RCA-026 logged.
**Tests passing:** 48/48 inline pure-function assertions (32 title-true + 9 title-false + 10 topic-anchor + 6 designation-consistency). Full pytest needs VPS run.
**Tests failing:** 0 new (expected).
**Blockers:** None. Wave 1 complete; Waves 2–5 listed in conversation log (AI-intensity scoring, non-AI cluster expansion, observability, evidence-span validation, two-stage classifier).

**Prior session 15 summary:** RCA-026 four-layer fix (commits `065e93e`, `3257350`) + 141-row backfill cleared "Applied ML" topics from sales/legal/HR/finance false-positives.

**Next action:** (1) Deploy Wave 1 to VPS: `git push` then `ssh a11yos-vps "cd /srv/roadmap && git pull && docker compose up -d --build --force-recreate backend"`. (2) Run full `pytest` on VPS to confirm Wave 1 tests pass (~50 new). (3) Re-run `scripts/backfill_rca026_non_ai.py --apply` — Wave 1 title patterns will catch additional rows missed by RCA-026 alone. (4) Decide on Wave 2 (AI-intensity scoring with word-boundary regex) based on what new false positives the admin queue surfaces in next 7 days. (5) Outstanding from prior sessions: rotate Gemini API key, `/summarize-jobs --status draft --limit 100`, bump `PROMPT_VERSION`, submit sitemap, set `INDEXNOW_KEY`.
