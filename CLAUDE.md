...
**Last session date:** 2026-04-22 (session 30 — SEO-08 BreadcrumbList JSON-LD)
**Last session summary (session 30):** Shipped SEO-08 (✅) in one commit (`f620a6a`). Added `BreadcrumbList` JSON-LD on the two routes that actually render visual breadcrumbs.

- **Audit-driven scope correction.** Spec listed 4 candidate routes; audit showed only 2 have server-side visual breadcrumbs to enhance: `/blog/{slug}` and `/jobs/{slug}`. `/profile/{user_id}` is JSON-API-only (SPA-rendered, [backend/app/routers/profile.py](backend/app/routers/profile.py)) — out of scope. `/verify/{credential_id}` ([backend/app/routers/verify.py](backend/app/routers/verify.py)) has no breadcrumb in `_render` — adding one would be UX scope creep beyond SEO-08's "enhance existing" intent.
- **/blog/{slug}** — second `<script type="application/ld+json">` block in the SEO-06 Jinja2 template at [backend/app/templates/blog/post.html](backend/app/templates/blog/post.html). Home → Blog → {title}; current-page item carries no `item` URL per Google spec.
- **/jobs/{slug}** — built `breadcrumb_ld` Python dict at [backend/app/routers/jobs.py:570-580](backend/app/routers/jobs.py#L570-L580) and serialized via `json.dumps()` mirroring the existing `JobPosting` JSON-LD pattern at line 580. **No Jinja2 migration needed for jobs.py** because the dict-then-dumps pattern never puts literal `{}` braces in the f-string source — already RCA-027-safe. Home → AI & ML Jobs → {job.title}.
- **Tests:** new `_extract_all_jsonld` helper for multi-block extraction; new `test_post_breadcrumb_list_json_ld_emitted` (blog) + new `test_per_job_ssr_includes_breadcrumblist_jsonld` (jobs); template-file structural assertion extended. Existing `test_per_job_ssr_includes_jobposting_jsonld` still passes — its non-greedy regex matches the FIRST `<script>` (JobPosting), so the second BreadcrumbList block coexists safely. 22/22 blog + jobs_public tests pass.
- **Phase 2 gates:** secrets scan clean, no TODO/FIXME introduced.
- **Live verification:** `/blog/01` returns Article + BreadcrumbList (3 crumbs, last has no `item` URL). `/jobs/principal-engineer-ai-inference-reliability-at-cerebrassystems-d884` returns JobPosting + BreadcrumbList (3 crumbs: Home → AI & ML Jobs → "Principal Engineer, AI Inference Reliability").
- **Docs updated:** [docs/SEO.md](docs/SEO.md) §0.2 (SEO-08 ✅) + Change log entry.

**Tests passing:** 441 (was 439 from session 28; +2 BreadcrumbList tests; 3 pre-existing unrelated failures from session 28 still present — not investigated this session).

**Session 29 context (preserved — happened between sessions 28 and 30 in parallel):** data-plane only, zero code/commits. Bulk-rejected 605 non-AI drafts via `scripts/backfill_rca026_non_ai.py --apply` + Phase 2/3 atomic SQL. Final counts: 549 drafts remaining (was 1,154), 642 rejected (was 37), 271 published. Audit markers: `last_reviewed_by='bulk-review 2026-04-22'` (320 rows) + `'bulk-review 2026-04-22-p3'` (285 rows). 101 `Other` drafts remain in queue including Anthropic pre-training / alignment / AI reliability SWE-s — admin manual pass needed (highest-value recoveries from Gemini mislabeling).

**Next action:** External Rich Results Test (<https://search.google.com/test/rich-results>) on `/blog/01` and one `/jobs/{slug}` URL — confirm both Article+BreadcrumbList and JobPosting+BreadcrumbList parse with zero errors. After that, GSC → Enhancements → Breadcrumbs should populate within 2 weeks. Unblocked next P0/P0-adj: SEO-07 (IndexNow — endpoint exists at [backend/app/routers/jobs.py:723](backend/app/routers/jobs.py#L723), needs `INDEXNOW_KEY` in `.env` + publish-event wiring across blog/jobs/cert routers + new `services/indexnow.py` helper), SEO-11 (OG image generator — would unblock the missing `og-default.png` SEO-06 references). Alternate track: session 29's 101-remaining-`Other`-drafts manual review.
**Open questions for the user:** None.

**Agent-utilization footer:**

- Opus: Phase 0 reads, route audit (4 → 2 — confirmed `/profile` is JSON-API-only and `/verify` has no breadcrumb), implementation across 4 files (~117 LOC), 22-test verification, commit + push + VPS rebuild + recreate, 2-URL live JSON-LD parse validation, docs updates.
- Sonnet: n/a — task was Sonnet-eligible per docs/SEO.md §0.3 but self-executed because all 4 files (template + jobs.py + 2 tests) stayed in Opus's hot read cache from session 28 (~117 LOC net). Cold-start cost would have outweighed token savings.
- Haiku: n/a — no multi-file sweeps; live URL validation was a single Python script across 2 URLs.
- codex:rescue: n/a — blog+jobs routers explicitly non-load-bearing per docs/SEO.md §1195; not security/auth/classifier-adjacent.
