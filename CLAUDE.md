...
**Last session date:** 2026-06-01 (session 53 cont. — per-company job alerts Phase 1)
**Last session summary (S53 cont.):** Shipped **per-company job-alert subscriptions + daily email digest** (`d7b43b8`, live, migration `20260601000000` applied). Users follow a company from any job page (or `/account` → "Companies you follow"); a daily 02:00 UTC cron emails each subscriber the new published jobs from companies they follow. Channel-agnostic schema (email live; telegram/whatsapp reserved in CHECK for Phase 2/3 — founder picked free-first + daily-digest). 13 files: migration `job_alert_subscriptions` (FK users CASCADE, unique(user,company,channel), additive) + model + `routers/job_alerts.py` (subscribe/unsubscribe/list, auth-gated, **check-then-insert idempotent** — the one bug tests caught was async `MissingGreenlet` from post-IntegrityError attr access) + `services/job_alerts_digest.py` + `scripts/send_job_alerts.py` (tz-aware watermark on /data) + cron.d + `frontend/job-subscribe.js` + account.html manage list. nginx unchanged (/api/ blanket proxy). **7 new tests pass, 37 jobs-suite green.** Smoke (public): asset 200, button renders, subscribe 401 anon, digest pipeline runs (0 subs → 0 sent, watermark set). codex:rescue n/a (additive migration + reused auth dep). **Pending: founder browser-smoke the logged-in toggle; Phase 2 Telegram / Phase 3 WhatsApp when ready; Brevo 300/day cap as subs grow.**

---

**Last session date:** 2026-05-31 (session 53 cont. — weekly Opus audit automation)
**Last session summary (S53 cont.):** Automated Layer 10 of the classifier defense — the weekly Opus classification audit, the only un-automated step (scheduler stamps ~1% Tier-1 published `audit.status=pending` weekly; the Opus review was a manual COPY-PROMPT, last run 2026-04-16 / 45 days stale). Shipped `e496e9a` (5 files, +303): `export_audit_jobs.py` (reuses the router's `_AUDIT_PROMPT_TEMPLATE` — single source of truth) + `import_audit_results.py` (per-row, flags mismatches into `admin_notes`, never auto-reclassifies) + `auto_audit_jobs.sh` (one `claude -p` pass on Max, $0) + `cron.d/auto_audit_jobs` (Mon 04:30 UTC) + logrotate. Scripts are volume-mounted so the pull went live without rebuild. Cron + logrotate installed; **export dry-run verified 25 pending, 76KB prompt builds correctly**; full E2E (Opus pass + verdict write) left for founder go or the Mon cron. Also diagnosed the "/admin/jobs all-zeros" question: ingest+summarize already cron'd; the zeros = admin review stalled 33 days (manual-by-design, can't automate). No RCA. codex:rescue n/a (no classifier logic). **Next: trigger first audit run (clears 45-day backlog) or wait for Mon 04:30; the 688-draft review stays manual.**
