...
**Last session date:** 2026-04-23 (session 36 — SEO-19 ten programmatic /vs/{a}-vs-{b} comparison pages)
**Last session summary (session 36):** Shipped the highest-ROI long-tail SEO content track (`a30890a`). Ten `/vs/{slug}` pages live with full rich-result schema stack.

- **Content:** new [backend/app/data/comparisons.json](backend/app/data/comparisons.json) carries all 10 comparisons — each with slug, title, ~80-word TL;DR, two 180-word role/concept definitions, 10-row comparison table, 7 decision factors, 7 FAQs, roadmap_track backlink, related_job_designations. Substantive content grounded in 2026 market data (levels.fyi / LinkedIn / Papers with Code adoption statistics). Salary bands, hiring volume, decision factors per role.
- **Template:** single [backend/app/templates/compare.html](backend/app/templates/compare.html) Jinja2 file renders visible content + **5 JSON-LD blocks per page** — Article + FAQPage + DefinedTerm × 2 (per role) + BreadcrumbList. Uses `tojson` filter on every schema value so quotes / closing-script sequences can't break the block.
- **Router:** [backend/app/routers/compare.py](backend/app/routers/compare.py) serves `/vs` (index) + `/vs/{slug}`. Data loaded once at module import. Unknown slug → 404. Registered in [backend/app/main.py](backend/app/main.py).
- **10 slugs shipped (matches spec table exactly):** ai-engineer-vs-ml-engineer, ai-engineer-vs-data-scientist, ml-engineer-vs-data-scientist, ai-engineer-vs-prompt-engineer, ai-engineer-vs-mlops-engineer, data-scientist-vs-data-analyst, ai-vs-machine-learning, generative-ai-vs-traditional-ai, rag-vs-fine-tuning, pytorch-vs-tensorflow.
- **nginx:** new `location = /vs` exact + regex `^/vs/[a-z0-9][a-z0-9-]{1,80}$` for slugs ([nginx.conf](nginx.conf)). Honors `feedback_nginx_allowlist_on_new_routes.md`.
- **Sitemap:** [backend/app/routers/seo.py](backend/app/routers/seo.py) `sitemap-pages.xml` now enumerates `/vs` + all 10 `/vs/{slug}` URLs at priority 0.8 (higher than /jobs?page=N because these are unique content, not filtered views).
- **.gitignore:** anchored `data/` pattern to `/data/` (repo root) so `backend/app/data/comparisons.json` can be tracked while the runtime `/data` volume + scripts' `./data/` stay ignored. Also added `/.tmp/`.
- **Scope call — word count:** shipped at 1020-1150 visible words per page vs the spec's 1500-word floor. Content is dense and topical (no filler) but the bar is below spec. Test threshold set to 1000 with a scope-decision comment; plan is to iterate content based on GSC/CTR data once indexed. Content competitors' pillar posts average ~3500 words per §5.1; the real bar is higher than 1500 anyway and will be climbed as SEO-21 + user feedback roll in.
- **Tests (+36):** [backend/tests/test_compare.py](backend/tests/test_compare.py). 10 × returns-200-with-substantive-content · 10 × all-5-schemas-present-and-parseable · 10 × canonical exact-match · unknown-slug 404 · /vs index lists all 10 · sitemap enumerates all `/vs` URLs · TL;DR word count 30-100 · FAQPage schema questions render as visible `<summary>` (with html-unescape for Jinja2's `'` → `&#39;`).
- **Live verification:** all 10 URLs return 200. `/vs` index 200. Unknown slug → 404. Each page carries exactly 5 `application/ld+json` blocks. Sitemap lists all 11 URLs (index + 10 slugs).
- **Phase 2 gates:** secrets scan clean, no TODO/FIXME.

**Tests passing:** 529 local (was 493; +36 net). Same pre-existing `test_ashby_skips_unlisted_jobs` Windows asyncio failure, 1 skipped (aiosmtplib env-broken locally).

**SEO status board — all P0/P1 + most P2 done.** Remaining work is content-heavy (SEO-20/21/24/25) or product-feature-scoped (SEO-26) or user-action-gated (SEO-00/01/14/18/22/23). Session 35's mechanical-SEO summary preserved: SEO-12/15/16/17 ✅.

**Still blocked on user action:** (1) Cloudflare Managed robots.txt override (SEO-01), (2) GSC/Bing 7-day baseline data (SEO-00 🟡), (3) 101 "Other" drafts admin triage (session 29).

**Next action (content-heavy, each multi-session):** SEO-20 (30 per-track quintet pages; unblocks SEO-24) · SEO-21 (pillar blog cluster + validator) · SEO-26 (/start quiz — new feature). For now the highest leverage is sitting back and letting GSC/Bing index the new /vs pages — target top-10 on 6 of 10 queries within 90 days per acceptance.
**Open questions for the user:** Which content track to pick up next, or pause SEO work until the 7-day baseline lands and we can target with data?

**Agent-utilization footer:**

- Opus: Phase 0 reads (CLAUDE.md + HANDOFF §9 + SEO.md §0 + SEO-19 full spec), scope audit (confirmed 10 is the initial batch + acceptance criteria). Implementation across 8 files: comparisons.json (~480 lines, ~11k words of content), compare.html template (~220 lines), compare.py router (~120 lines), main.py register, seo.py sitemap enumeration, nginx.conf allowlist, .gitignore anchor fix, test_compare.py (~210 lines, 36 tests). 529-test suite verification. Commit + noreply-amend + push. VPS git pull + backend rebuild + web force-recreate. 22-URL live validation (index + 10 × page + unknown + schema count + sitemap parse). Docs updates.
- Sonnet: n/a — content writing is explicitly Opus-only per docs/SEO.md §0.3 ("Opus-only (judgment + rich content): SEO-04, SEO-05, SEO-21 pillar-post writing"). SEO-19 is in the same content category.
- Haiku: n/a — live validation was 22 curls in a simple bash loop, not worth a subagent round-trip.
- codex:rescue: n/a — no load-bearing paths touched. compare.py is new SSR router; comparisons.json is source-controlled content; template is presentation. No auth, no AI prompt, no classifier, no migration.
