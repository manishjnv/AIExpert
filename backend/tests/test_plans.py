...
async def test_enroll_creates_active_plan():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        resp = await c.post("/api/plans", json={"goal": "generalist", "duration": "6mo", "level": "intermediate"}, cookies={"session": token})
        assert resp.status_code == 200
        data = resp.json()
...
async def test_re_enroll_archives_old_plan():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        # First enrollment
        resp1 = await c.post("/api/plans", json={"goal": "generalist", "duration": "6mo", "level": "intermediate"}, cookies={"session": token})
        plan1_id = resp1.json()["id"]

        # Re-enroll
        resp2 = await c.post("/api/plans", json={"goal": "generalist", "duration": "6mo", "level": "intermediate"}, cookies={"session": token})
...
async def test_active_plan_all_checks_false():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        resp = await c.get("/api/plans/active", cookies={"session": token})
        assert resp.status_code == 200
        data = resp.json()
...
async def test_progress_tick_persists():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        # Tick a checkbox
        resp = await c.patch("/api/progress", json={"week_num": 1, "check_idx": 0, "done": True}, cookies={"session": token})
        ...
        # Uncheck
        resp = await c.patch("/api/progress", json={"week_num": 1, "check_idx": 0, "done": False}, cookies={"session": token})
        assert resp.status_code == 204
...
async def test_migrate_progress():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        resp = await c.post("/api/progress/migrate", json={"progress": blob}, cookies={"session": token})
        assert resp.status_code == 200
        data = resp.json()
