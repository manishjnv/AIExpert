...
### 037 — "Fail-fast" fix for silent-drop bug just relocated the failure to a runtime crash (2026-04-26) [Anti-pattern / fix shape]

- **Symptom (chain):** A live test of the new weekly digest showed the "New courses" section was empty even though the LangChain MCP template was `published`. Root cause was a silent contract gap in `set_template_status()`: it only stamped `last_reviewed_on` if a `reviewer_name` was provided, and `_recent_courses` filtered on that stamp — so a publish-without-reviewer produced a row that the digest silently dropped. **First-attempt fix made `set_template_status` raise `ValueError` if status='published' and no `reviewer_name` was supplied.** That looked correct in isolation (the same code path can no longer create the bad data), but it converted a "silent missing section" failure mode into a "runtime ValueError on every legacy / script / direct-meta-edit publish path" failure mode. The user pushed back: "fix this permanently" — meaning eliminate the silent drop, not relocate it.
- **Root cause (of the wrong fix shape):** I conflated "make this exact code path safe" with "make the system correct regardless of how data arrives at the consumer." The publish helper is one of several ways data lands in `_meta.json` — direct edits, migration scripts, future admin endpoints, or parallel sessions can all bypass it. A strict raise in one writer only catches that writer; every other path remains broken. Worse, if the strict raise lands in production and is then hit by a script that previously worked, the digest stops running entirely (the new failure is *louder* than the old one but no more correct, and it adds blast radius).
- **Fix:** [backend/app/curriculum/loader.py:234-264](../backend/app/curriculum/loader.py#L234-L264) — reverted the strict raise. `set_template_status('published', …)` is permissive again (no `reviewer_name` requirement) but ALWAYS stamps `last_reviewed_on` on a publish transition (the original gap). `last_reviewed_by` is only stamped when a real reviewer name is supplied — no auto-attributed identity, audit honesty preserved. The `publish_template()` front door still requires `admin_name` for the public-facing API, but the lower-level helper intentionally stays lenient so it can never itself be the cause of a silent drop. [backend/app/services/weekly_digest.py:486-535](../backend/app/services/weekly_digest.py#L486-L535) — `_recent_courses` now resolves the publish date through three sources: (1) `_meta.json` `last_reviewed_on`, (2) the template JSON file's mtime, (3) skip the row. Even legacy / direct-edited entries that ended up published without a stamp self-heal at read time via the filesystem timestamp. Two regression tests at [backend/tests/test_publish_gate.py::test_set_template_status_published_always_stamps_date](../backend/tests/test_publish_gate.py) (verifies stamp lands with empty / None / real reviewer_name) and [backend/tests/test_weekly_digest.py::test_recent_courses_falls_back_to_file_mtime_when_stamp_missing](../backend/tests/test_weekly_digest.py) (builds an isolated stamp-less published template, asserts mtime fallback surfaces it).
- **Prevention:**
  - **A "fail-fast" fix for a silent-drop bug is the wrong shape when the data has multiple writers.** It only catches THIS writer; the bug recurs the moment any other path produces the same shape. The right fix has two layers: (a) make the canonical writer always produce correct data, and (b) make the consumer self-heal from legacy / edge data. Both, not just (a). The combination is robust against future writers we haven't predicted.
  - **Before adding a strict raise to a previously-permissive function, audit every caller AND every data path that writes to the same store.** If there's any path that doesn't go through this function (direct file edit, migration, script, parallel API endpoint), the raise is incomplete and shifts blast radius rather than reducing it.
  - **When the user pushes back on a fix with "fix this permanently," they almost always mean "the failure mode should be impossible, not just loud."** Re-frame: what makes the symptom impossible regardless of how the system got here? That's usually a read-path defense (fallback, normalization, healing) plus a write-path improvement, not a write-path lock.
  - **Add a regression test for the read-path self-heal too**, not just the write-path stamp. The healing only matters when the canonical write path was bypassed — which is exactly the case the unit-tested write path can't exercise.
...
| Strict `raise` to "fix" a silent-drop bug with multi-writer data | High | Adding `raise` to one writer of a shared store only catches that writer; every other path (direct file edit, migration script, parallel API endpoint, future caller) silently drops the same shape. Right fix has TWO layers: (a) canonical writer always produces correct data, (b) consumer self-heals from legacy/edge data. When user says "fix permanently" → make the failure mode impossible regardless of how the system got there, not just loud. Read-path defense + write-path improvement, not write-path lock. |
