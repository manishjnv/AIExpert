...
| SEO-21 | P1 | Pillar blog cluster with 10-point validator-enforced quality bar | 🟡 2 of 6 posts live (2026-04-25). **Live:** [/blog/03-ai-engineer-vs-ml-engineer](https://automateedge.cloud/blog/03-ai-engineer-vs-ml-engineer) (q6 — 3134w, Article+FAQPage+DefinedTerm) + [/blog/04-learn-ai-without-cs-degree-2026](https://automateedge.cloud/blog/04-learn-ai-without-cs-degree-2026) (q7 — 3000+w, Article+FAQPage+HowTo+DefinedTerm). HowTo template wire-up shipped session 40 ([post.html](../backend/app/templates/blog/post.html) emits `HowTo` JSON-LD when `payload.how_to.steps` present). New CLI publish path [scripts/stage_blog_draft.py](../scripts/stage_blog_draft.py) replaces manual paste-into-textarea — `--all` mode is idempotent. RCA-031 fixed in same session (Dockerfile was missing `COPY data ./data`, so trusted_sources.json never reached the container — pillar validator was inert in prod since session 38). 4 posts remaining (q2 next, then q4, intent-gap q3/q4, intent-gap q10). |
...
- **2026-04-25** — SEO-21 posts 03 + 04 published live + SEO-21 publishing infra shipped + RCA-031 (silent prod bug) fixed. **Live URLs:** [/blog/03-ai-engineer-vs-ml-engineer](https://automateedge.cloud/blog/03-ai-engineer-vs-ml-engineer) (q6, HTTP 200, 47 KB, JSON-LD: Article + BreadcrumbList + FAQPage[10] + DefinedTermSet[4]) and [/blog/04-learn-ai-without-cs-degree-2026](https://automateedge.cloud/blog/04-learn-ai-without-cs-degree-2026) (q7, HTTP 200, 54 KB, JSON-LD: same four + **HowTo**). IndexNow auto-fired on each publish (per [blog.py:670](../backend/app/routers/blog.py#L670)). Blog index `/blog` lists 4 cards in newest-first order (04 → 03 → 02 → 01). Confirmed live `"@type": "HowTo"` emission on post 04 — proves the schema wire-up shipped this session is functioning end-to-end. **HowTo template wire-up** (commit `16a37cc`): [post.html](../backend/app/templates/blog/post.html) emits `<script type="application/ld+json">` HowTo block when `payload.how_to.steps` is present, gated so existing posts without `how_to` are unaffected. [blog.py:443](../backend/app/routers/blog.py#L443) `_render_post` signature gains `how_to` kwarg, threaded from payload. **CLI publish path** (commit `c42b794`, sys.path fix `314748d`): new [scripts/stage_blog_draft.py](../scripts/stage_blog_draft.py) runs `validate_payload` + `save_draft` directly inside the backend container, replacing the manual paste-into-textarea step. `--all` mode is idempotent — scans `/app/blog-archives/*.json` (mounted from `./docs/blog/`), skips already-published, overwrites already-drafted with re-validation. Admin still clicks Publish in `/admin/blog` (no auto-publish, by design — content stays human-gated). Workflow: `ssh a11yos-vps "cd /srv/roadmap && docker compose exec -T backend python scripts/stage_blog_draft.py --all"`. **RCA-031 — pillar validator inert in prod since session 38** (commit `4162362`, RCA in [docs/RCA.md:233](./RCA.md#L233)): the SEO-25 `backend/data/trusted_sources.json` allowlist file was created in session 38 but `backend/Dockerfile` never gained a `COPY data ./data` instruction. Validator's `_TRUSTED_SOURCES_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "trusted_sources.json"` resolved to `/app/data/trusted_sources.json` inside the container — correct path, missing file. Result: every "ok=True" pillar validation since SEO-25 shipped was **local-only**. The container would have rejected every pillar with "trusted_sources.json not found" — invisible bug because no pillar post had been pasted into `/admin/blog` to trigger it. Surfaced when [scripts/stage_blog_draft.py](../scripts/stage_blog_draft.py) ran the validator inside the container for the first time. Fix: one-line `COPY data ./data` in [backend/Dockerfile:34](../backend/Dockerfile#L34) between `COPY app ./app` and `COPY alembic.ini ./`. Image rebuilt + force-recreated. New prevention pattern row added to [docs/RCA.md:257](./RCA.md#L257): "Any new directory under `backend/` that runtime code reads must be added to the Dockerfile in the same PR. Validate by running the consumer inside the container, not just locally." **Status board:** SEO-21 moved 🟡 (1 of 6 authored, 0 live) → 🟡 (2 of 6 live). 4 posts remain — q2 next, then q4, intent-gap q3/q4, intent-gap q10. **Editorial drift to flag for next session:** post 04 came back with 18 paragraphs >4 sentences and 29 sentences >30 words (post 03 had 8 + 10) — tighten the next post in authoring rather than retrofitting. **Adjacent (non-SEO) ship in same session:** reject-reason `prompt()` → labelled `<select>` modal in [admin_jobs.py:1010](../backend/app/routers/admin_jobs.py#L1010) (commit `742592f`). **Agent-utilization:** Opus = staging CLI authoring + Dockerfile diagnosis + deploy + live verification + RCA entry + SEO.md/HANDOFF/§9 updates. Sonnet: n/a (single-author UI/script work, not parallel-eligible). Haiku: n/a (sub-5-call sweeps cheaper as direct Opus tool calls). codex:rescue: deferred — no auth/AI-classifier/Alembic/jobs path touched; engagement point remains SEO-26 (`quiz_outcomes` migration).
...
- **2026-09-20** — CTR + crawlability fix on `/` from Bing Webmaster data (3 months: 1,878 impressions, 10 clicks, 0.53% CTR; top query "ai generalist career roadmap" 157 impressions at position 5.9, 0 clicks). **Title** `AI Learning Roadmap — Interactive Study Plan & Progress Tracker` → `Free AI Generalist Roadmap: 24-Week Career Study Plan` (53 chars; matches the top three query stems: generalist / career / roadmap). **Meta description** 200 → 141 chars so it is no longer cut mid-sentence. **Single h1:** the generated scaffold heading is now `<h2>` and week headings `<h3>` ([scripts/generate_roadmap_scaffold.py](../scripts/generate_roadmap_scaffold.py)); the hero `<h1 id="heroTitle">` is the only h1 (Bing flagged 2). **Crawlable links:** the scaffold now ends with a `<nav>` of 10 real `<a href>` links (`/roadmap`, 5 track hubs, `/blog`, `/jobs`, `/vs`, `/leaderboard`); before this the raw HTML had zero internal links because the nav is injected by `nav.js`. Rule 8 preserved (no build step, file:// still works; the scaffold is removed when JS runs). og:* and JSON-LD untouched. **Sitemap 502** was not a sitemap bug: see RCA-036 (nginx stale upstream IP; every proxied route was down ~13 h). Caveat for reading results: many Bing queries are long machine-style strings that look like AI-assistant grounding queries, which rarely produce clicks, so CTR will improve less than raw impressions suggest. Re-check Bing in 2 weeks. Tests: 9 SEO tests pass; generator `--check` clean.
