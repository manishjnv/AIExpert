...
| SEO-01 | P0 | robots.txt (Disallow /admin /api /account /share/ /og/) | ✅ done (2026-04-21) |
...
| SEO-04 | P0 | SSR content scaffold for landing page (preserves rule 8) | ⬜ pending |
| SEO-05 | P0 | Course + ItemList + FAQPage JSON-LD on landing (full property set) | ⬜ pending |
| SEO-06 | P0 | Article JSON-LD on every blog post | ⬜ pending |
| SEO-07 | P0 | Activate IndexNow (set key, wire publish events) | ⬜ pending |
| SEO-08 | P1 | BreadcrumbList JSON-LD everywhere breadcrumbs render | ⬜ pending |
| SEO-09 | P1 | Blog RSS feed at /blog/feed.xml | ⬜ pending |
...
| SEO-13 | P1 | Missing canonicals on blog index / profile / leaderboard | ⬜ pending |
...
| SEO-24 | P1 | Hub ItemList schema listing all roadmap tracks | ⬜ pending |
| SEO-25 | P1 | Trusted-sources allowlist + E-E-A-T enforcement in blog validator | ⬜ pending |
| SEO-26 | P2 | /start interactive quiz landing with personalized plan output | ⬜ pending |

**Next action** (always — source of truth for what to pick up next): SEO-00. Nothing else can be measured without it. Once SEO-00 lands, unlocks parallel execution across SEO-01, SEO-03, SEO-06, SEO-07 (all independent).
...
- **2026-04-21** — SEO-01 shipped (done). Created [frontend/robots.txt](../frontend/robots.txt) with the 5 Disallow rules + Sitemap directive. Added `location = /robots.txt` block in [nginx.conf](../nginx.conf) between favicon and JS/CSS location blocks — uses `try_files /robots.txt =404` to serve from the frontend volume, 24h cache, explicit text/plain content-type. Deploy: frontend + nginx.conf changes both need to land on VPS; nginx needs `docker compose exec nginx nginx -s reload` (or `docker compose restart nginx`) to pick up the location block — the robots.txt file itself is volume-mounted so no rebuild.
